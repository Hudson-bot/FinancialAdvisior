...
const SavingsAI = ({ onClose }) => {
  const apiKey = import.meta.env.VITE_OPENROUTER_API_KEY;

  const [messages, setMessages] = useState([
    {
      text: "Hi! I'm your Savings Assistant. Ask me anything about saving money, budgeting, or financial planning.",
      sender: 'ai',
    }
  ]);
      ...
      >
        <div className="bg-[#4B1F52] text-white p-4 rounded-t-xl flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold">Savings AI Assistant</h3>
          </div>
              ...
              <div key={index} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div
                  className={`max-w-[80%] p-3 rounded-lg ${
                    msg.sender === 'user' ? 'bg-[#4B1F52] text-white' : 'bg-gray-100'
                  }`}
                >
                  {msg.text}
                </div>
